...
class BottleneckBlock(nn.Module):
    def __init__(self, in_channels, out_channels, adapter_dim):
        ...
        self.adapter_proj_down = nn.Conv2d(in_channels, adapter_dim, kernel_size=3)
        self.adapter_proj_up = nn.Conv2d(adapter_dim, out_channels, kernel_size=1)
        self.act = nn.GELU()
        self.norm = nn.LayerNorm(out_channels)  # Optional normalization for stability
        nn.init.kaiming_uniform_(self.adapter_proj_down.weight, a=math.sqrt(5))
        nn.init.zeros_(self.adapter_proj_up.weight)
    ...
    def forward(self, x):
        ...
        # print(x1.shape)
        x1 = self.norm(x1.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # LayerNorm expects (N, H, W, C)
...
class BottleneckTransformer(nn.Module): 
    def __init__(self, in_channels, out_channels, adapter_dim) : 
        ...
        self.attention = Transformer2DModel(
            attention_head_dim=4,  # Number of attention heads
            num_attention_heads=in_channels // 32,  # Compute heads based on channel count
            in_channels=in_channels // 8,  # Number of input channels
            num_layers=1,  # Number of transformer layers
            norm_num_groups=4,  # Normalization factor (typically for LayerNorm or GroupNorm)
            use_linear_projection=True,  # Whether to use linear projection in attention
            upcast_attention=True,  # Whether to upcast attention computation for stability
        )
        self.adapter_proj_down = nn.Conv2d(in_channels, adapter_dim, kernel_size=3, padding=1)
        self.adapter_proj_up = nn.Conv2d(adapter_dim, out_channels, kernel_size=1)
        self.act = nn.GELU()
        self.norm = nn.LayerNorm(out_channels)  # Optional normalization for stability
        nn.init.kaiming_uniform_(self.adapter_proj_down.weight, a=math.sqrt(5))
...
class StackedBottleneckAdapter(nn.Module):
    def __init__(self, in_channels, out_channels, adapter_channel, num_bottleneck_blocks=7):
        super(StackedBottleneckAdapter, self).__init__()
        self.bottleneck_blocks = nn.ModuleList(
            [BottleneckTransformer(in_channels, out_channels, adapter_channel) for _ in range(num_bottleneck_blocks)]
        )
...
class Adapter(nn.Module):
    def __init__(self, in_features, out_features, channel_reduction, adapter_dim=128, num_heads=1, dropout=0.0, adapter_type="bottleneck"):
        """
        Args:
            in_features (int): Input dimensionality (size of the embedding).
            out_features (int): Output dimensionality (size of the output embedding).
        ...
        if adapter_type == "bottleneck":
            self.adapter = StackedBottleneckAdapter(in_features, out_features, in_features//channel_reduction, num_heads)
...
class UNetMidBlock2DCrossAttnMusic(nn.Module):
    def __init__(
        ...
        self.adapter = Adapter(in_channels, in_channels, 8, adapter_dim=128, num_heads=2, adapter_type=adapter_type)
        ...
        for _ in range(num_layers):
            ...
            # adapters_p.append(Adapter(in_channels, in_channels, 8, adapter_dim=128, num_heads=9, adapter_type=adapter_type))
    ...
    def forward(
        ...
        hidden_states = self.resnets[0](hidden_states, temb)
        # hidden_states = self.adapters_p[0](hidden_states)
...
class CrossAttnDownBlock2DMusic(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            # adapters.append(Adapter(in_channels, in_channels, 8, adapter_type=adapter_type))
            if not dual_cross_attention:
                attentions.append(
                    Transformer2DModel(
                        attn_num_head_channels,
        ...
        self.adapter = Adapter(out_channels, out_channels, 8, adapter_type=adapter_type, num_heads=2)
